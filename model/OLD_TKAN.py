...
class Model:
      def __init__(self):
            ...
            self.X_train_seq = np.random.rand(100, 10, 5)
            self.y_train_seq = np.random.rand(100, 5)

            self.model = keras.Sequential([
                  keras.layers.InputLayer(shape=self.X_train_seq.shape[1:]),
                  ...
                  keras.layers.Dense(self.y_train_seq.shape[1]),
            ])

      def compile_execute_test(self):
            self.model.compile(optimizer='adam', loss='mse')
            self.model.fit(self.X_train_seq, self.y_train_seq, epochs=10, verbose=1)
